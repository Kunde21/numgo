...
func TestPowC(t *testing.T) {
	t.Parallel()
	a := Arange(0, 40, 2)
	if b := a.PowC(0).Equals(FullArray64(1, 21)); !b.All().At(0) {
		t.Log(b)
		t.Log(a)
		t.Fail()
	}
...
func TestDiv(t *testing.T) {
	...
	a = a.C().Div(a)
	if e := a.Equals(FullArray64(1, 200)); e.All().At(0) {
		t.Log(e)
		t.Fail()
	}
...
func TestPow(t *testing.T) {
	...
	a = a.Reshape(2, 25, 4).Pow(FullArray64(0, 4))
	if e := a.Equals(FullArray64(1, 2, 25, 4)); !e.All().At(0) {
		t.Log(e)
		t.Fail()
	}
	if a.HasErr() {
...
func TestValRith(t *testing.T) {
	...
	for i, v := range []struct {
		...
		{NewArray64(nil, 0), &Array64{nDimMetadata{err: InvIndexError}, nil}, InvIndexError, "Var in error not caught"},
